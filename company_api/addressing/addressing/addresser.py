import enum
import hashlib

FAMILY_NAME = 'COMPANY_SAWTOOTH'
FAMILY_VERSION = '1.0'
NAMESPACE = hashlib.sha512(FAMILY_NAME.encode('utf-8')).hexdigest()[:6]

USER_PREFIX = '00'
EMPLOYEE_PREFIX = '10'
COMPANY_PREFIX = '01'


@enum.unique
class AddressSpace(enum.IntEnum):
    USER_PREFIX = 0
    EMPLOYEE_PREFIX = 2
    COMPANY_PREFIX = 1
    OTHER_FAMILY = 100


def get_user_address(public_key):
    return NAMESPACE + USER_PREFIX + hashlib.sha512(
        public_key.encode('utf-8')).hexdigest()[:62]


def get_employee_address(employee_id):
    return NAMESPACE + EMPLOYEE_PREFIX + hashlib.sha512(
        employee_id.encode('utf-8')).hexdigest()[:62]


def get_company_address(company_id):
    return NAMESPACE + COMPANY_PREFIX + hashlib.sha512(
        company_id.encode('utf-8')).hexdigest()[:62]


def get_address_type(address):
    if address[:len(NAMESPACE)] != NAMESPACE:
        return AddressSpace.OTHER_FAMILY

    infix = address[6:8]

    if infix == '00':
        return AddressSpace.USER_PREFIX
    if infix == '10':
        return AddressSpace.EMPLOYEE_PREFIX
    if infix == '01':
        return AddressSpace.COMPANY_PREFIX
    return AddressSpace.OTHER_FAMILY
