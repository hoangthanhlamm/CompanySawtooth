...
class Messenger(object):
    ...
    async def send_create_user_transaction(self, private_key, username, role, timestamp):
    ...
    async def send_create_com_transaction(self, private_key, com_id, name, timestamp):
        transaction_signer = self._crypto_factory.new_signer(
            secp256k1.Secp256k1PrivateKey.from_hex(private_key)
        )

        batch = make_create_com_transaction(
            transaction_signer=transaction_signer,
            batch_signer=self._batch_signer,
            com_id=com_id,
            name=name,
    ...
    async def send_update_com_transaction(self, private_key, com_id, establish, address, timestamp):
        transaction_signer = self._crypto_factory.new_signer(
            secp256k1.Secp256k1PrivateKey.from_hex(private_key)
        )

        batch = make_update_com_transaction(
            transaction_signer=transaction_signer,
            batch_signer=self._batch_signer,
            com_id=com_id,
            establish=establish,
    ...
    async def send_create_emp_transaction(self, private_key, emp_id, name, timestamp):
        transaction_signer = self._crypto_factory.new_signer(
            secp256k1.Secp256k1PrivateKey.from_hex(private_key)
        )

        batch = make_create_emp_transaction(
            transaction_signer=transaction_signer,
            batch_signer=self._batch_signer,
            emp_id=emp_id,
            name=name,
    ...
    async def send_update_emp_transaction(self, private_key, emp_id, age, address, email, com_id, timestamp):
        transaction_signer = self._crypto_factory.new_signer(
            secp256k1.Secp256k1PrivateKey.from_hex(private_key)
        )

        batch = make_update_emp_transaction(
            transaction_signer=transaction_signer,
            batch_signer=self._batch_signer,
            emp_id=emp_id,
            age=age,
